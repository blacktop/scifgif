...
func GetImageByPath(path string) (ImageMetaData, error) {
	ctx := context.Background()
	...
	termQuery := elastic.NewTermQuery("path", path)
	searchResult, err := client.Search().
		Index("scifgif"). // search in index "twitter"
		Query(termQuery). // specify the query
		Size(1).          // take documents 0-9
	...
	return ImageMetaData{}, fmt.Errorf("image path %s not found", path)
}
